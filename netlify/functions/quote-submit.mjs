...
function displayLocation(locRaw) {
 const loc = String(locRaw || "").trim();
  if (!loc) return "Naples / Fort Myers area";
...
export default async (request, context) => {
 ...
 if (!answers.location || !String(answers.location).trim()) {
      answers.location = "five service cities (referral)";
 }
 ...
 return json({ error: PUBLIC_EMAIL_FAILURE, code: "IDEMPOTENCY_IN_FLIGHT" }, 503);
 }
