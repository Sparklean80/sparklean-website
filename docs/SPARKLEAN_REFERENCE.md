...
Last updated: **2026-08-28** (Careers FAQ dark styling; public `/careers`; noindex `/careers/apply`)
...
**Public Careers hiring flow (2026-08-28):** `/careers` is in the sitewide top nav (after Why Sparklean; also in the hamburger). It is the indexable careers landing. It loads published jobs only from `https://api.sparklean.co` and injects `JobPosting` JSON-LD for those jobs (no invented dates). Empty-state wording is exactly: “There are no open positions at this time. Please check back soon.” The Applying to Sparklean FAQ uses dark/gold accordion styling (no default white browser buttons). `/careers/apply` is a public application URL (not founder-gated) but stays `noindex, nofollow`, out of the sitemap, and `Disallow` in `robots.txt` (plus `X-Robots-Tag: noindex, nofollow`). `/careers/offer/:token` and `/careers/documents/:token` stay `noindex`, unlisted, and are refused by `hiring-applicant-gate` unless Sparklean OS accepts the token (401 without a token, 404 if invalid). Founder review gates and demo answers are removed from production. Identity-document files are captured on Sparklean OS (`/hiring/onboarding/...`) and must never pass through Netlify; if private R2/S3 is not configured (`HIRING_OBJECT_BUCKET` and related Render vars), document collection stays unavailable with a clear message and no website fallback. Office hiring controls remain on `office.sparklean.co` / `/api/office/hiring/*`. Careers pages use shared homepage header/footer chrome (`css/sparklean-site-header.css?v=menu-1` for hamburger/menu). `npm run test:site` is read-only against `api.sparklean.co`. A synthetic live application is DESTRUCTIVE and requires `SPARKLEAN_LIVE_HIRING_MUTATION=1` in a local non-CI shell (`npm run test:careers:live-mutation` refuses to write). Tony publishes jobs from Office and sets Render storage vars separately — do not publish a job or enable I-9/DL collection from this repo.
