...
Last updated: **2026-08-28** (`/careers` lists Residential Cleaner full-time and part-time openings; apply stays noindex)
...
**Google reviews CTA (2026-08-19):** Do not use a `/maps/place/` URL with a street slug. Sparklean is a service-area listing; that slug opened an empty Bonita Springs map (no reviews panel). `GOOGLE_REVIEWS_HREF` is Maps search `Sparklean Cleaning Bonita Springs` (no street). `test:html-seo` blocks Bernwood/`24221`/Brink Cir. About quote CTAs use `/contact?quote=1#quote-intake`. Direct `cid`/`placeid` URL only if extracted from a working Share link — do not invent IDs.
...
**Public Careers hiring flow (2026-08-28):** `/careers` is in the sitewide top nav (after Why Sparklean; also in the hamburger). It is the indexable careers landing. Two openings are listed on the page itself: **Residential Cleaner — Full-Time** ($18/hour, $27/hour OT after 40, Mon–Fri, report by 7:00 AM) and **Residential Cleaner — Part-Time** ($18/hour, 20–30 hours/week, no guaranteed overtime). Empty-state copy remains in the DOM but stays hidden while those listings are shown. If `GET https://api.sparklean.co/api/hiring/openings` returns published jobs, those cards replace the HTML listings and `JobPosting` JSON-LD uses live IDs/`posted_at` (no invented dates). Static `JobPosting` JSON-LD on `/careers` has no `datePosted`. `/careers/apply` is a public application URL (not founder-gated) but stays `noindex, nofollow`, out of the sitemap, and `Disallow` in `robots.txt` (plus `X-Robots-Tag: noindex, nofollow`). Apply start still needs a live OS `job_id`; `?role=full-time` / `?role=part-time` match a published opening when one exists. `/careers/offer/:token` and `/careers/documents/:token` stay `noindex`, unlisted, and are refused by `hiring-applicant-gate` unless Sparklean OS accepts the token (401 without a token, 404 if invalid). Founder review gates and demo answers are removed from production. Identity-document files are captured on Sparklean OS (`/hiring/onboarding/...`) and must never pass through Netlify; if private R2/S3 is not configured (`HIRING_OBJECT_BUCKET` and related Render vars), document collection stays unavailable with a clear message and no website fallback. Careers pages use shared homepage header/footer chrome. `npm run test:site` is read-only against `api.sparklean.co`. A synthetic live application is DESTRUCTIVE and requires `SPARKLEAN_LIVE_HIRING_MUTATION=1` in a local non-CI shell (`npm run test:careers:live-mutation` refuses to write). Do not enable I-9/DL collection from this repo. The Applying to Sparklean FAQ uses dark/gold accordion styling (no default white browser buttons).
