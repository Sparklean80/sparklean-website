...
**Mobile hero + breadcrumb (2026-08-18):** City pages had a CSS typo grouping `.breadcrumb` with `.pp-img{height:220px}` — created a tall empty black band with Home › Residential › City. Fixed on all five city LPs; mobile heroes taller with `object-position:center 20%`. Homepage mobile/tablet: use landscape `aspect-ratio` (3/2 → 4/3) instead of tall `vw` heights that vertically compress the Roxy wipe photo; `object-position` biased left so Roxy stays in frame.
