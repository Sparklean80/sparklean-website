...
**Lead / conversion reconciliation boundary (2026-08-12, review `review/lead-conversion-boundary`, lease/outbox correction):** Opaque ETag `onlyIfNew`/`onlyIfMatch`; claim lease + orphan reclaim after crash-before-create; material-hash binding (`IDEMPOTENCY_MATERIAL_CONFLICT`); durable Brevo outbox (`PENDING`/`SENDING`/`DELIVERED`); never `pendingHydration` 200 for missing leads. BlobsServer + fresh-module restart proofs: `test:idempotency-lease`, `test:blob-concurrency`. Evidence work note updated. `npm run test:funnel`.
