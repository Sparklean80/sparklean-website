...
Last updated: **2026-08-28** (`/careers` body layout: centered hero, two job cards, benefit blocks, accordion cards)
...
**Public Careers hiring flow (2026-08-28):** `/careers` is in the sitewide top nav (after Why Sparklean; also in the hamburger). It is the indexable careers landing. Body layout (header/footer unchanged): centered hero; two-column **Residential Cleaner — Full-Time** and **Part-Time** cards (pay, schedule, location, requirements, “Applications opening soon”); three “What working at Sparklean means” blocks (direct employment, supervised teams, consistent standards); application information as black-and-gold accordion cards. No floating empty-state box. Cards stack on mobile. Static `JobPosting` JSON-LD has no `datePosted`. `/careers/apply` stays `noindex, nofollow`, out of the sitemap, and `Disallow` in `robots.txt` (plus `X-Robots-Tag: noindex, nofollow`). Apply start still needs a live OS `job_id`; `?role=full-time` / `?role=part-time` match a published opening when one exists. `/careers/offer/:token` and `/careers/documents/:token` stay `noindex`, unlisted, and are refused by `hiring-applicant-gate` unless Sparklean OS accepts the token (401 without a token, 404 if invalid). Founder review gates and demo answers are removed from production. Identity-document files are captured on Sparklean OS (`/hiring/onboarding/...`) and must never pass through Netlify. `npm run test:site` is read-only against `api.sparklean.co`. Do not enable I-9/DL collection from this repo.
