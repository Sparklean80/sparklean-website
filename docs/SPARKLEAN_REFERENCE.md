...
**Lead / conversion reconciliation boundary (2026-08-12, review `review/lead-conversion-boundary`, correction):** First-party `js/sparklean-attribution.js` preserves click ids when Ads/gtag blocked; contact + intake call `conversion-report` directly if SparkleanAds absent. Blob stores **hash only** of `reportToken` (timing-safe verify); version/CAS writes; BlobsServer concurrency tests. `fireAndReportConversion` requires durable report ok before treating outcome as BROWSER_SENT. Reconcile: never authorize on `x-netlify-event` alone; HTTP needs `SPARKLEAN_RECONCILE_KEY`. Abuse: origin gate, rate limit, idempotency. Schedule `*/15` in `netlify.toml` + function config. Evidence work note updated. Tests: `npm run test:funnel` (ads, paid-intake, leads-store, adversarial, blob-concurrency).
