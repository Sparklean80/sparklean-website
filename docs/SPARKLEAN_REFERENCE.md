...
**Lead / conversion reconciliation boundary (2026-08-12, review `review/lead-conversion-boundary`, CONVERSION_BOUNDARY_CORRECTION):** Opaque ETag conditional writes (`onlyIfNew` / `onlyIfMatch`) with no unconditional fallback; atomic idempotency claim+seal before lead create/Brevo (losers hydrate); monotonic tracking-state matrix (BROWSER_SENT / OFFLINE_IMPORTED cannot regress); append-only attempt race + simultaneous contact/quote proofs on real BlobsServer. First-party attribution + durable conversion-report still required for BROWSER_SENT. Evidence: `docs/work-notes/2026-08-12-conversion-boundary-evidence.md`. Tests: `npm run test:funnel`.
